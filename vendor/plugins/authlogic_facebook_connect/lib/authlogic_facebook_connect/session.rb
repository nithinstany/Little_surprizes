module AuthlogicFacebookConnect
  module Session
    ...
    module Config
      ...
      #
      #
      # * <tt>Default:</tt> :session_key
      # * <tt>Accepts:</tt> Symbol
      def session_key_field(value = nil)
        rw_config(:session_key_field, value, :session_key)
      end
      alias_method :session_key_field=, :session_key_field
    ...
    module Methods
      ...
      def validate_by_facebook_connect
        ...
        self.attempted_record = facebook_user_class.find(:first, :conditions => { facebook_id_field => facebook_session.user.uid })

        if self.attempted_record
          self.attempted_record.send(:"#{session_key_field}=", facebook_session.session_key)
        ...
        unless self.attempted_record || facebook_skip_new_user_creation
          begin
            ...
            if klass == facebook_user_class
              #new_user.send(:"#{facebook_id_field}=", facebook_session.user.uid)
              #new_user.send(:"#{session_key_field}=", facebook_session.session_key)
              new_user = klass.new
              new_user.send(:"#{facebook_id_field}=",facebook_session.user.uid)
              new_user.send(:"#{session_key_field}=",facebook_session.session_key) 
            else
              new_user.send(:"build_#{facebook_user_class.to_s.underscore}", :"#{facebook_id_field}" => facebook_session.user.uid, :"#{session_key_field}" => facebook_session.session_key)
            end
      ...
      def session_key_field
        self.class.session_key_field
      end
